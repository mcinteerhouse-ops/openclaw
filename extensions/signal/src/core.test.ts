...
import {
  ...
  setSignalContainerReceiveCheckForTest,
...
afterEach(() => {
  ...
  setSignalContainerReceiveCheckForTest(undefined);
...
describe("signal setup parsing", () => {
  ...
  it("reuses an existing Signal container published host port", async () => {
    ...
    vi.spyOn(fetchRuntime, "resolveFetch").mockReturnValue(fetchImpl as unknown as typeof fetch);
    const receiveCheck = vi.fn(async () => ({ ok: true as const, status: 101, error: null }));
    setSignalContainerReceiveCheckForTest(receiveCheck);
    ...
    expect(receiveCheck).toHaveBeenCalledWith("http://127.0.0.1:19080", 5_000, "+15555550123");
  });

  it("reuses an existing Signal container published on IPv6 loopback", async () => {
    execFileAsyncMock.mockImplementation(async (_cmd: string, args: string[]) => {
      switch (args[0]) {
        case "version":
          return { stdout: "27.0.0\n", stderr: "" };
        case "container":
          ...
          if (args[3] === '{{json (index .NetworkSettings.Ports "8080/tcp")}}') {
            return {
              stdout: JSON.stringify([{ HostIp: "::1", HostPort: "19080" }]),
              stderr: "",
            };
          }
          throw new Error(`unexpected docker inspect format: ${args.join(" ")}`);
    ...
    const fetchImpl = vi.fn(async (url: string) => {
      if (url.includes("127.0.0.1")) {
        throw new Error(`unexpected IPv4 loopback fetch: ${url}`);
      }
      if (url === "http://[::1]:19080/v1/about") {
        return new Response(JSON.stringify({ versions: ["v1", "v2"], mode: "json-rpc" }), {
          status: 200,
          headers: { "Content-Type": "application/json" },
        });
      }
      if (url === "http://[::1]:19080/v1/accounts") {
        return new Response(JSON.stringify(["+15555550123"]), {
          status: 200,
          headers: { "Content-Type": "application/json" },
        });
    ...
    await expect(
      ...
      ok: true,
      account: "+15555550123",
      httpUrl: "http://[::1]:19080",
      containerName: "openclaw-signal",
      volumeName: "openclaw-signal",
      qrLink: undefined,
    });
    expect(receiveCheck).toHaveBeenCalledWith("http://[::1]:19080", 5_000, "+15555550123");
  });

  it("rejects single-account Signal containers when receive is unavailable", async () => {
    execFileAsyncMock.mockImplementation(async (_cmd: string, args: string[]) => {
      switch (args[0]) {
        case "version":
          return { stdout: "27.0.0\n", stderr: "" };
    ...
    vi.spyOn(fetchRuntime, "resolveFetch").mockReturnValue(fetchImpl as unknown as typeof fetch);
    const receiveCheck = vi.fn(async () => ({
      ok: false as const,
      status: null,
    ...
    await expect(
      ...
      ok: false,
      error:
        "Signal receive check failed for +15555550123: Signal container receive WebSocket timed out",
    });
    expect(receiveCheck).toHaveBeenCalledWith("http://127.0.0.1:19080", 5_000, "+15555550123");
  });

  it("rejects selected Signal container accounts when receive is unavailable", async () => {
    execFileAsyncMock.mockImplementation(async (_cmd: string, args: string[]) => {
      switch (args[0]) {
        case "version":
          return { stdout: "27.0.0\n", stderr: "" };
    ...
    const fetchImpl = vi.fn(async (url: string) => {
      ...
      if (url === "http://127.0.0.1:19080/v1/accounts") {
        return new Response(JSON.stringify(["+15555550123", "+15555550124"]), {
          status: 200,
          headers: { "Content-Type": "application/json" },
        });
      }
    ...
    const receiveCheck = vi.fn(async () => ({
      ok: false as const,
      status: null,
      error: "Signal container receive endpoint did not upgrade to WebSocket (HTTP 404)",
    }));
    setSignalContainerReceiveCheckForTest(receiveCheck);
    const prompts = createQueuedWizardPrompter({ selectValues: ["+15555550124"] });
    ...
    await expect(
      ...
      ok: false,
      error:
        "Signal receive check failed for +15555550124: Signal container receive endpoint did not upgrade to WebSocket (HTTP 404)",
    });
    expect(receiveCheck).toHaveBeenCalledWith("http://127.0.0.1:19080", 5_000, "+15555550124");
  });

  it("rejects newly linked Signal container accounts when receive is unavailable", async () => {
    execFileAsyncMock.mockImplementation(async (_cmd: string, args: string[]) => {
      switch (args[0]) {
        case "version":
          return { stdout: "27.0.0\n", stderr: "" };
    ...
    const fetchImpl = vi.fn(async (url: string) => {
      ...
      if (url === "http://127.0.0.1:18080/v1/accounts") {
        accountReads += 1;
        return new Response(JSON.stringify(accountReads === 1 ? [] : ["+15555550123"]), {
          status: 200,
          headers: { "Content-Type": "application/json" },
        });
      }
      if (
        ...
        return new Response("sgnl://linkdevice?uuid=abc&pub_key=def", {
          status: 200,
          headers: { "Content-Type": "text/plain" },
        });
      }
    ...
    const receiveCheck = vi.fn(async () => ({
      ok: false as const,
      status: null,
      error: "Signal container receive WebSocket closed before open (1008)",
    }));
    setSignalContainerReceiveCheckForTest(receiveCheck);
    ...
    await expect(
      ...
      ok: false,
      error: [
        "Signal receive check failed for +15555550123: Signal container receive WebSocket closed before open (1008)",
        "",
        "OpenClaw already created or started Docker container openclaw-signal.",
        "Run setup again to reuse it, or remove it with:",
        "docker rm -f openclaw-signal",
    ...
    expect(receiveCheck).toHaveBeenCalledWith("http://127.0.0.1:18080", 5_000, "+15555550123");
  });

  it("rejects a stopped existing Signal container published on a public interface before start", async () => {
    execFileAsyncMock.mockImplementation(async (_cmd: string, args: string[]) => {
      switch (args[0]) {
        case "version":
          return { stdout: "27.0.0\n", stderr: "" };
        case "container":
          ...
          if (args[3] === '{{json (index .NetworkSettings.Ports "8080/tcp")}}') {
            return {
              stdout: JSON.stringify([{ HostIp: "0.0.0.0", HostPort: "19080" }]),
              stderr: "",
            };
          }
          throw new Error(`unexpected docker inspect format: ${args.join(" ")}`);
        case "start":
          throw new Error("unsafe container should not be started");
        default:
          throw new Error(`unexpected docker args: ${args.join(" ")}`);
      }
    });
    ...
    await expect(
      ...
      ok: false,
      error: [
        "Docker container openclaw-signal publishes container port 8080/tcp on 0.0.0.0, not loopback.",
        "Managed Signal Docker setup only reuses containers bound to 127.0.0.1 or ::1.",
        "Remove it with:",
        "docker rm -f openclaw-signal",
        "or choose the existing server setup path if you intentionally manage that container.",
      ].join("\n"),
    });
    expect(execFileAsyncMock).not.toHaveBeenCalledWith(
      "docker",
      ["start", "openclaw-signal"],
      expect.any(Object),
    );
  ...
  it("resets stale container mode for non-interactive native setup", () => {
    ...
    expect(next?.channels?.signal?.httpUrl).toBeUndefined();
  });
