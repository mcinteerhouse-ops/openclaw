...
type SignalContainerReceiveCheck = typeof containerCheck;

let signalContainerReceiveCheckForTest: SignalContainerReceiveCheck | undefined;

/** @internal Exported for testing. */
export function setSignalContainerReceiveCheckForTest(
  check: SignalContainerReceiveCheck | undefined,
): void {
  signalContainerReceiveCheckForTest = check;
}
...
function normalizeSignalContainerBaseUrl(hostPort: number, hostIp = "127.0.0.1"): string {
  return hostIp === "::1" ? `http://[::1]:${hostPort}` : `http://127.0.0.1:${hostPort}`;
}

type SignalContainerPublishedPort =
  | { ok: true; hostPort: number; hostIp: string }
  | { ok: false; reason: "missing" }
  | { ok: false; reason: "non-loopback"; hostIp: string };

function isLoopbackDockerHostIp(hostIp: string): boolean {
  return hostIp === "127.0.0.1" || hostIp === "::1";
}

function parseSignalContainerPublishedPort(stdout: string): SignalContainerPublishedPort {
  ...
  let nonLoopbackHostIp: string | null = null;
  for (const binding of bindings) {
    ...
    if (!isLoopbackDockerHostIp(hostIp)) {
      nonLoopbackHostIp = hostIp || "<empty>";
      continue;
    }
    const parsedPort = Number(hostPort);
    if (Number.isSafeInteger(parsedPort) && parsedPort >= 1 && parsedPort <= 65535) {
      return { ok: true, hostPort: parsedPort, hostIp };
    }
  }
  return nonLoopbackHostIp
    ? { ok: false, reason: "non-loopback", hostIp: nonLoopbackHostIp }
    : { ok: false, reason: "missing" };
}
...
> {
  ...
  const publishedPort = parseSignalContainerPublishedPort(inspect.stdout);
  if (!publishedPort.ok) {
    if (publishedPort.reason === "non-loopback") {
      return {
        ok: false,
        error: [
          `Docker container ${containerName} publishes container port 8080/tcp on ${publishedPort.hostIp}, not loopback.`,
  ...
  return { ok: true, hostPort: publishedPort.hostPort, hostIp: publishedPort.hostIp };
}
...
async function verifySignalContainerReceive(params: {
  httpUrl: string;
  account: string;
}): Promise<{ ok: true } | { ok: false; error: string }> {
  const receiveCheck = signalContainerReceiveCheckForTest ?? containerCheck;
  const result = await receiveCheck(params.httpUrl, 5_000, params.account);
  if (result.ok) {
    return { ok: true };
  }
  const detail =
    result.error ?? (typeof result.status === "number" ? `HTTP ${result.status}` : "unknown error");
  return {
    ok: false,
    error: `Signal receive check failed for ${params.account}: ${detail}`,
  };
}

async function buildSignalContainerSetupSuccess(params: {
  account: string;
  httpUrl: string;
  containerName: string;
  volumeName: string;
  qrLink?: string;
  managedContainerThisRun: boolean;
}): Promise<SignalContainerSetupRunnerResult> {
  const receiveReady = await verifySignalContainerReceive({
    httpUrl: params.httpUrl,
    account: params.account,
  });
  if (!receiveReady.ok) {
    return params.managedContainerThisRun
      ? withManagedContainerNotice(receiveReady, params.containerName)
      : receiveReady;
  }
  return {
    ok: true,
...
export async function defaultSignalContainerSetupRunner(
  ...
  const httpUrl = normalizeSignalContainerBaseUrl(hostPort, hostIp);
  ...
  if (initialAccounts.accounts.length === 1) {
    return await buildSignalContainerSetupSuccess({
      account: initialAccounts.accounts[0],
      httpUrl,
      containerName: params.containerName,
      volumeName,
      qrLink,
      managedContainerThisRun,
    });
  }
  if (initialAccounts.accounts.length > 1) {
    const account = await params.prompter.select({
