...
function buildSignalSetupPatch(input: {
  ...
  const externalDaemonPatch = input.httpUrl
    ? { autoStart: false, apiMode: "auto" as const }
    : {};
  const nativeDaemonPatch =
    !input.httpUrl && (input.cliPath || input.httpHost || input.httpPort)
      ? { autoStart: true, apiMode: "native" as const, httpUrl: undefined }
      : {};
  return {
...
function patchSignalSetupConfigForAccount(params: {
  ...
  const existingDefault = preservedAccounts[DEFAULT_ACCOUNT_ID] ?? {};
